...
int get_length_atribut_baju(string atribut, list<Baju> list_baju){
    ...
    }else if(atribut == "kategori_produk"){
        length = 15;
        for(it = list_baju.begin(); it != list_baju.end(); it++){
            if(it->get_kategori_produk().length() > length){
                length = it->get_kategori_produk().length();
            }
        }
...
void tabel_baju(list<Baju> list_baju){
    ...
    int length_kategori_produk = get_length_atribut_baju("kategori_produk", list_baju);
    ...
    cout << string(length_id + length_nama_produk + length_kategori_produk + length_harga_produk + length_jenis + length_warna + length_bahan + length_untuk + length_size + length_merk + 11, '-') << endl;
    cout << "|ID" << string(length_id - 2, ' ') 
    << "|Nama Produk" << string(length_nama_produk - 11, ' ')
    << "|Kategori Produk" << string(length_kategori_produk - 15, ' ')
    ...
    for(it = list_baju.begin(); it != list_baju.end(); it++){
        ...
        << "|" << it->get_kategori_produk() << string(length_kategori_produk - it->get_kategori_produk().length(), ' ')
...
int main(){
    ...
    Baju baju;
    baju.addBaju(1, "Baju_Anjing_Sporty", "Pakaian_Anjing", 75000, "Jersey", "Polyester", "Merah", "Anjing", "M", "PetStyle");
    list_baju.push_back(baju);
    baju.addBaju(2, "Sweater_Kucing_Winter", "Pakaian_Kucing", 90000, "Sweater", "Wool", "Biru", "Kucing", "S", "MeowWear");
    list_baju.push_back(baju);
    baju.addBaju(3, "Jaket_Anjing_Waterproof", "Pakaian_Anjing", 120000, "Jaket", "Nylon", "Hitam", "Anjing", "L", "PawFashion");
    list_baju.push_back(baju);
    baju.addBaju(4, "Daster_Kucing_Floral", "Pakaian_Kucing", 65000, "Daster", "Katun", "Pink", "Kucing", "M", "FurryChic");
    list_baju.push_back(baju);
    baju.addBaju(5, "Baju_Tidur_Hamster_Lucu", "Pakaian_Hamster", 85000, "Piyama", "Satin", "Kuning", "Hamster", "XS", "ComfyPet");
    list_baju.push_back(baju);

    tabel_baju(list_baju);
    int jumlah_baju = 0;
    int id = 0;
    string nama_produk, kategori_produk, jenis, warna, bahan, untuk, size, merk;
    ...
    for(int i = 0; i < jumlah_baju; i++){
        ...
        cin >> size;
        cin >> merk;
        baju.addBaju(id, nama_produk, kategori_produk, harga_produk, jenis, warna, bahan, untuk, size, merk);
        list_baju.push_back(baju);
    }
